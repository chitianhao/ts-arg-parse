...
main(int, const char **argv)
{
  ...
  // two basic commands
  ts::ArgParser::Command &init_command   = top_command.add_subcommand("init", "initialize traffic blabla", "HOME", 1);
  ts::ArgParser::Command &remove_command = top_command.add_subcommand("remove", "remove traffic blabla", "REMOVE_ENV", 0);
  ...
  init_command.add_subcommand("subinit2", "sub initialize traffic blabla", "", 2).add_option("--abc", "-x", "some testing", "", 1);
  ...
  // ts::ArgParser::Command &somecommand = top_command.get_subcommand("asdf");
